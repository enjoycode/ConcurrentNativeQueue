...
所有内存（段头结构体 + 槽位数组）均通过 `NativeMemory` 分配，**零 GC 压力、零托管堆分配**；段满时自动分配新段链接到尾部（容量指数增长），槽位消费完后立即释放，段头在 `Dispose` 时统一回收。
...
- **分段链表** — 段按需分配并链接，容量从初始值指数增长（上限 1M），段满时 O(1) 创建新段，无需全局暂停或缓冲区迁移
- **全 Native 内存** — 段头结构体与槽位数组均通过 `NativeMemory` 分配，不产生任何托管堆分配，`ConcurrentNativeQueue<T>` 本身也是 unmanaged 类型
- **两阶段回收** — 槽位数组在段消费完后由消费者立即释放；段头延迟到 `Dispose` 统一释放（生产者可能持有旧段指针，这是无锁 native 设计中的核心生命周期问题）
...
| `ConcurrentNativeQueue(int segmentSize)` | 以指定初始段大小创建队列（最小为 2，后续段容量指数增长至上限 1M） |
...
队列由原生段组成，每个段包含一个 `NativeMemory` 分配的槽位数组。段容量从初始值指数增长（×2 直至上限 1M），减少段切换频率。每个槽位有 `State` 字段标记写入状态：
...
### 段生命周期（全 Native 两阶段回收）

段头结构体与槽位数组均通过 `NativeMemory` 分配，无任何托管对象参与。
生命周期的核心挑战是：**生产者可能在任意时刻被抢占并持有旧段指针**，因此段头不能在消费后立即释放。

1. **段满** — 生产者检测到 `offset >= capacity`，通过 CAS 创建新段（容量翻倍）并链接到 `Next`，然后推进共享 `_tail` 指针。若 CAS 失败（另一个生产者先创建了），立即释放多余段（该段从未被发布，无并发风险）
2. **段消费完毕** — 消费者检测到 `offset >= capacity`，**立即释放槽位数组**（安全：所有 `State == 1` 意味着所有生产者已写完），前进到 `Next` 段。段头结构体保留
3. **Dispose** — 从 `_origin`（首段）遍历整条链表，释放所有段头结构体和剩余槽位数组
4. **段头开销** — 每个段头约 160 字节。指数增长使段头数量为对数级别：处理 10 亿条数据约 1000 个段头 ≈ 200KB，可忽略
